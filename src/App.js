...
export default function App() {
  ...
  const getStats = (client) => {
    const data = {
      'Client A': { filing: 100, audit: 90, color: '#10b981' },
      'Client B': { filing: 45, audit: 60, color: '#f59e0b' },
      'Client C': { filing: 10, audit: 5, color: '#ef4444' }
    };
    return data[client] || data['Client A'];
  };
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '30px' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', padding: '30px', borderRadius: '24px', border: '1px solid #222' }}>
              ...
              <div style={{ marginBottom: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', marginBottom: '5px' }}><span>GST Filing</span><span>{stats.filing}%</span></div>
                <div style={{ width: '100%', height: '8px', background: '#222', borderRadius: '10px', overflow: 'hidden' }}>
                  <div style={{ width: ${stats.filing}%, height: '100%', background: stats.color, transition: '0.5s' }} />
                </div>
              </div>

              <div style={{ marginBottom: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', marginBottom: '5px' }}><span>Audit Ready</span><span>{stats.audit}%</span></div>
                <div style={{ width: '100%', height: '8px', background: '#222', borderRadius: '10px', overflow: 'hidden' }}>
                  <div style={{ width: ${stats.audit}%, height: '100%', background: stats.color, transition: '0.5s' }} />
                </div>
              </div>
            </div>
